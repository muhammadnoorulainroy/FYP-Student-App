...
class Parent_SetNewPassword : AppCompatActivity() {
    var Phone_No: String? = ""
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_parent_set_new_password)

        Phone_No = intent.getStringExtra("phone") ?: ""
        new_password = findViewById((R.id.psnnew_password))
        conf_password = findViewById((R.id.psnconfirm_password))
        progressBar = findViewById((R.id.psn_progress_bar))
    ...
    fun setNewPasswordBtn(view: View) {
        ...
        var database: DatabaseReference = FirebaseDatabase.getInstance().getReference("Parents")
        database.child(Phone_No!!).child("password").setValue(new_pass)

        //Calling New Activity
        val intent = Intent(applicationContext, Parent_FP_SuccessMessage::class.java)
